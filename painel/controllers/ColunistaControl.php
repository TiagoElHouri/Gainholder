	...
	**************************************/
	...
	class ColunistaControl extends Controller{
		...
		public function index($pg = 1){
			...
			$dados['colunistas'] = ''; //$this->listAdministradores($pg);
		...
		public function atualizar($id){
			if($id != null AND is_numeric($id)){
				...
				if (isset($_REQUEST['modulo']) && $_REQUEST['modulo'] == "administrador") {
					...
					$edicao = $this->colunistaModel->update($dados['login'],$id);
		...
		public function editar($id,$param = null){
			...
			if($id != null AND is_numeric($id)){
				...
				$administrador = $this->colunistaModel->getRow('l.id as loginId, u.id as usuarioId, email, u.nome as nomeUsuario, status, permissoes','WHERE l.id ='.$id);
		...
		public function verificaEmail($emailAdmin, $emailAdminEdit, $emailAdminRef){
			...
			if(!empty($emailAdmin)){
				...
				$administrador = $this->colunistaModel->getRow("l.id as loginId, u.id as usuarioId, l.email","WHERE l.email ='".$emailAdmin."' ");
				...
				$administrador = $this->colunistaModel->getRow("l.id as loginId, u.id as usuarioId, l.email","WHERE l.email ='".$emailAdminEdit."' ");
